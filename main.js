...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (exportOptionsModalEl) {
        ...
        // Listener for the existing "Generate .zip File" button
        document.getElementById('confirm-export-zip-btn').addEventListener('click', async () => {
            const checkboxes = exportOptionsModalEl.querySelectorAll('input[type="checkbox"]');
            const exposedProperties = Array.from(checkboxes).filter(cb => cb.checked).map(cb => cb.name);
        ...
        // Listener for the new "Export Skydimo LUA" button
        const exportSkydimoBtn = document.getElementById('export-skydimo-lua-btn');
        if (exportSkydimoBtn) {
            exportSkydimoBtn.addEventListener('click', () => {
                const exportModal = bootstrap.Modal.getInstance(exportOptionsModalEl);
                if (exportModal) exportModal.hide();
                generateAndDownloadSkydimoLua();
            });
        }

        // Function to build and download the Skydimo Lua script
        function generateAndDownloadSkydimoLua() {
            // Ensure live objects are in sync with the form
            updateObjectsFromForm();
            
            const effectTitle = getControlValues()['title'] || 'MyEffect';
            const safeFilename = effectTitle.replace(/[\s/?%*:|"<>\\]/g, '_');

            // Construct the advanced Skydimo LUA structure
            let luaCode = `-- Skydimo LUA Script generated by RGBJunkie Effect Builder\n`;
            luaCode += `-- Effect Name: ${effectTitle}\n\n`;

            // Lua helper functions to calculate and render the animated gradients
            luaCode += `function lerp(a, b, t)\n    return a + (b - a) * t\nend\n\n`;
            luaCode += `function lerp_color(c1, c2, t)\n    local r = math.floor(lerp(c1.r, c2.r, t))\n    local g = math.floor(lerp(c1.g, c2.g, t))\n    local b = math.floor(lerp(c1.b, c2.b, t))\n    return {r=r, g=g, b=b}\nend\n\n`;

            luaCode += `function get_gradient_color(progress, stops)\n`;
            luaCode += `    local t = progress % 1.0\n`;
            luaCode += `    if t < 0 then t = t + 1.0 end\n`;
            luaCode += `    if #stops == 0 then return {r=255, g=0, b=255} end\n`;
            luaCode += `    if #stops == 1 then return stops[1] end\n`;
            luaCode += `    if t <= stops[1].position then return stops[1] end\n`;
            luaCode += `    if t >= stops[#stops].position then return stops[#stops] end\n`;
            luaCode += `    for i = 1, #stops - 1 do\n`;
            luaCode += `        local s1 = stops[i]\n`;
            luaCode += `        local s2 = stops[i+1]\n`;
            luaCode += `        if t >= s1.position and t <= s2.position then\n`;
            luaCode += `            local range = s2.position - s1.position\n`;
            luaCode += `            if range == 0 then return s1 end\n`;
            luaCode += `            local amount = (t - s1.position) / range\n`;
            luaCode += `            return lerp_color(s1, s2, amount)\n`;
            luaCode += `        end\n    end\n    return stops[#stops]\nend\n\n`;

            luaCode += `local objects = {\n`;
            objects.forEach(obj => {
                let stopsStr = "{}";
                if (obj.gradient && obj.gradient.stops && obj.gradient.stops.length > 0) {
                    stopsStr = "{\n";
                    obj.gradient.stops.forEach(stop => {
                        const rgba = parseColorToRgba(stop.color);
                        stopsStr += `      {position = ${stop.position}, r = ${rgba.r}, g = ${rgba.g}, b = ${rgba.b}},\n`;
                    });
                    stopsStr += "    }";
                } else {
                    stopsStr = "{{position = 0, r = 255, g = 255, b = 255}}";
                }

                luaCode += `  {\n`;
                luaCode += `    shape = "${obj.shape}",\n`;
                luaCode += `    x = ${Math.round(obj.x)},\n`;
                luaCode += `    y = ${Math.round(obj.y)},\n`;
                luaCode += `    width = ${Math.round(obj.width)},\n`;
                luaCode += `    height = ${Math.round(obj.height)},\n`;
                luaCode += `    animSpeed = ${(obj.animationSpeed || 0)},\n`;
                luaCode += `    scrollDir = "${obj.scrollDirection || 'right'}",\n`;
                luaCode += `    stops = ${stopsStr}\n  },\n`;
            });
            luaCode += `}\n\n`;

            luaCode += `function begin_frame(env)\nend\n\n`;

            // Map standard LED layout back to a 320x200 canvas configuration
            luaCode += `function get_color(x, y, width, height, env)\n`;
            luaCode += `  local final_r, final_g, final_b = 0, 0, 0\n\n`;
            luaCode += `  -- Map Skydimo's LED matrix coordinates back to the builder's 320x200 canvas system\n`;
            luaCode += `  local cx = (width > 1) and (x / (width - 1)) * 320 or 160\n`;
            luaCode += `  local cy = (height > 1) and (y / (height - 1)) * 200 or 100\n\n`;
            
            luaCode += `  for i, obj in ipairs(objects) do\n`;
            luaCode += `    local is_inside = false\n`;
            luaCode += `    local local_x = cx - obj.x\n`;
            luaCode += `    local local_y = cy - obj.y\n\n`;
            luaCode += `    if obj.shape == "rectangle" or obj.shape == "text" or obj.shape == "pixel-art" or obj.shape == "audio-visualizer" or obj.shape == "tetris" then\n`;
            luaCode += `      if local_x >= 0 and local_x <= obj.width and local_y >= 0 and local_y <= obj.height then\n`;
            luaCode += `        is_inside = true\n`;
            luaCode += `      end\n`;
            luaCode += `    elseif obj.shape == "circle" or obj.shape == "ring" then\n`;
            luaCode += `      local radius = obj.width / 2\n`;
            luaCode += `      local dx = local_x - radius\n`;
            luaCode += `      local dy = local_y - (obj.height / 2)\n`;
            luaCode += `      if (dx * dx + dy * dy) <= (radius * radius) then\n`;
            luaCode += `        is_inside = true\n`;
            luaCode += `      end\n`;
            luaCode += `    end\n\n`;

            luaCode += `    if is_inside then\n`;
            luaCode += `      local progress = 0\n`;
            luaCode += `      if obj.scrollDir == "right" then progress = local_x / obj.width\n`;
            luaCode += `      elseif obj.scrollDir == "left" then progress = 1.0 - (local_x / obj.width)\n`;
            luaCode += `      elseif obj.scrollDir == "down" then progress = local_y / obj.height\n`;
            luaCode += `      elseif obj.scrollDir == "up" then progress = 1.0 - (local_y / obj.height) end\n`;
            luaCode += `      \n`;
            luaCode += `      progress = progress - (env.time * obj.animSpeed * 0.1)\n`;
            luaCode += `      local col = get_gradient_color(progress, obj.stops)\n`;
            luaCode += `      final_r, final_g, final_b = col.r, col.g, col.b\n`;
            luaCode += `    end\n`;
            luaCode += `  end\n\n`;

            luaCode += `  return rgb(final_r, final_g, final_b)\n`;
            luaCode += `end\n`;
            ...
            try {
                ...
                link.download = `${safeFilename}.lua`;
