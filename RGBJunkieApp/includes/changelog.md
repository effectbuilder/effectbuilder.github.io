...
- **LED Studio got a refresh** — **Settings → Colors → LED Studio** now arranges its controls into clean, evenly-aligned cards (Component, Brush, Mode, Actions) so nothing looks cramped or ragged. It also adds quick color presets, live painted/total counts, clearer Paint and Erase toggles, and a brighter layout preview that matches the rest of the app.
