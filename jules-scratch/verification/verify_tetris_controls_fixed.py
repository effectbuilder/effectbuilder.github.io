...
# This is the full body of the getDefaultObjectConfig function extracted from main.js
get_default_object_config_function_body = """
(newId) => {
    return [
        // Geometry & Transform
...
async def main():
    async with async_playwright() as p:
        ...
        try:
            ...
            # Inject the function into the page's context
            await page.add_script_tag(content=f"window.getDefaultObjectConfig = {get_default_object_config_function_body}")

            # Change the shape of the first object to 'tetris' to make the controls visible
            await page.select_option('select[name="obj1_shape"]', 'tetris')
            await asyncio.sleep(0.5) # Wait for UI to update

            # --- Start of tests ---

            # 1. Test "Comet" Animation
            print("Testing 'Comet' animation...")
            await page.select_option('select[name="obj1_tetrisAnimation"]', 'comet')
            await asyncio.sleep(1)

            # 2. Test "Comet Gravity" Animation
            print("Testing 'Comet Gravity' animation...")
            await page.select_option('select[name="obj1_tetrisAnimation"]', 'comet-gravity')
            await asyncio.sleep(1)

            # 3. Test "Comet Gravity Reversed" Animation
            print("Testing 'Comet Gravity Reversed' animation...")
            await page.select_option('select[name="obj1_tetrisAnimation"]', 'comet-gravity-reversed')
            await asyncio.sleep(1)

            # 4. Test "Blur Edges" Checkbox
            print("Testing 'Blur Edges'...")
            blur_edges_checkbox = page.locator('input[name="obj1_tetrisBlurEdges"]')
            await expect(blur_edges_checkbox).to_be_visible()
            await blur_edges_checkbox.check()
            await asyncio.sleep(1)
            await blur_edges_checkbox.uncheck()

            # 5. Test "Hold" Checkbox
            print("Testing 'Hold'...")
            hold_checkbox = page.locator('input[name="obj1_tetrisHold"]')
            await expect(hold_checkbox).to_be_visible()
            await hold_checkbox.check()
            await asyncio.sleep(2) # Hold for 2 seconds to observe the effect
            await hold_checkbox.uncheck()
            ...
            # Take a final screenshot regardless of success or failure
