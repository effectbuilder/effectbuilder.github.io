 ...
 *
 * **Multipart (schema v3)** — preferred: field `metadata` (JSON) + `file_logs_zip` (`rgbjunkie-logs.zip`).
 * **Multipart (schema v2)** — legacy: individual file uploads:
 *   file_rgbjunkie_log, file_hardware_snapshot, file_rgb_hardware_debug, file_rgbjunkie_log_prev
 *
...
function handle_multipart_v2(): void
{
    ...
    if (
        $schemaVersion >= 3
        && isset($_FILES['file_logs_zip']['tmp_name'])
        && is_string($_FILES['file_logs_zip']['tmp_name'])
        && $_FILES['file_logs_zip']['tmp_name'] !== ''
        && is_uploaded_file($_FILES['file_logs_zip']['tmp_name'])
    ) {
        $attachments['rgbjunkie-logs.zip'] = $_FILES['file_logs_zip']['tmp_name'];
    } else {
        $attachDefs = [
            'file_rgbjunkie_log' => 'rgbjunkie.log',
            'file_hardware_snapshot' => 'hardware-snapshot.txt',
            'file_rgb_hardware_debug' => 'rgb-hardware-debug.txt',
            'file_freeze_events' => 'freeze-events.txt',
...
{
    $items = [];
    if (support_attachment_flag_true($att, 'logs_zip')) {
        $zb = isset($att['logs_zip_bytes']) ? (int) $att['logs_zip_bytes'] : 0;
        $detail = ($zb > 0) ? ('Included · ' . format_byte_size($zb)) : 'Included · size not reported';
        $items[] =
            '<li style="margin:6px 0;"><strong>rgbjunkie-logs.zip</strong> — ' . $h($detail) . '</li>';
        $inner = [
            ['flag' => 'rgbjunkie_log', 'label' => 'rgbjunkie.log'],
            ['flag' => 'hardware_snapshot', 'label' => 'hardware-snapshot.txt'],
            ['flag' => 'rgb_hardware_debug', 'label' => 'rgb-hardware-debug.txt'],
            ['flag' => 'freeze_events', 'label' => 'freeze-events.txt'],
        ];
        foreach ($inner as $row) {
            if (!support_attachment_flag_true($att, $row['flag'])) {
                continue;
            }
            $items[] =
                '<li style="margin:2px 0 2px 18px;font-size:13px;color:#444;">'
                . $h($row['label']) . ' inside zip</li>';
        }
    ...
    $baseRows = [
        ...
        ['flag' => 'rgb_hardware_debug', 'label' => 'rgb-hardware-debug.txt', 'bytes_key' => 'rgb_hardware_debug_bytes'],
        ['flag' => 'freeze_events', 'label' => 'freeze-events.txt', 'bytes_key' => 'freeze_events_bytes'],
    ];
